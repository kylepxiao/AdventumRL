...
class CameraDQNAgent(Agent):
    ...
    def queryActions(self, frame, world_state, current_r ):
        ...
        current_s = self.processFrame(frame.pixels, frame.width, frame.height, current_s)
            ...
            if is_first_action:
                ...
                while True:
                    ...
                    if world_state.is_mission_running and len(world_state.observations)>0 and not world_state.observations[-1].text=="{}" and world_state.number_of_video_frames_since_last_state > 0:
                        ...
                        total_reward += self.act(frame, world_state, current_r)
                        break
...
if __name__ == "__main__":
    ...
    agent = CameraDQNAgent(mission_file, quest_file)
    try:
